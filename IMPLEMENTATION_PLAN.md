...
v1.6 · 2026-09-23 · **Phase 0 complete · Phase 1 built · Phase 2 started (Sports, real Contact details, Instagram)**
...
## Update: the opening scroll experience — the moving footage, then the 3D turf — now plays on phones too, not just laptops

You asked for the site to look and feel the same on a phone as it does on a laptop, specifically calling out the opening scroll story: "the video should not get paused, and the text should not get messy." Two separate things were going on, and both are fixed now.

**The real bugs, which existed regardless of this request.** On a phone, the opening headline ("WE BUILD the ground YOU MOVE ON.") was being cut off at the edge of the screen and overlapping the round SCROLL button — the button's fallback size was a fixed size tuned for a laptop, and on a narrow phone screen it came out nearly half the screen wide. That's fixed, and checked on four phone sizes from a small older phone up to a large modern one.

**The bigger decision: does the phone get the animated scroll-through-the-turf effect and the 3D cross-section, or does it stay on the simpler picture-based version?** This was a deliberate choice made earlier in the project, after measuring that the heavy version stutters badly on phones and computers without a graphics card. You asked to bring the full version to phones anyway, accepting that trade-off, so that's what we built:

- The opening scroll — footage playing as you scroll, then (where the phone's own graphics chip supports it) the 3D turf opening into a cross-section — now plays on phones exactly as it does on a laptop, decided by what the *device* can actually do, never by how wide the screen is.
- Bringing it to a narrow screen surfaced several more of the same class of bug the headline had — text and labels positioned for a wide screen running into each other or off the edge on a narrow one. All of the ones we found are fixed: the depth-gauge labels ("THE ARENA" / "THE SURFACE" / "THE FIBRE"), the brand line ("SLORA creates surfaces…"), the "Every Game Begins with the Ground" line, the cross-section's layer labels (GRASS FIBRES, INFILL, BACKING, BASE, DRAINAGE — these were the worst affected, showing only a small gold dot with no visible name at all on a phone), and the "ILLUSTRATIVE · NOT TO SCALE" note, which was landing on top of the chat button.
- **Two related sections were left as they were, on purpose, because this request was specifically about the opening scroll:** the Sports section's sideways-sliding row of seven panels, and the "How We Build" glass video panel, both still use the lighter, phone-friendly version they always have. Say the word if you want either of those brought to phones too — it is the same kind of decision, just for a different part of the site.
- **Something worth knowing honestly:** this computer has no graphics card of its own, so everything above was checked for correct layout (nothing cut off, nothing overlapping, no errors) on software-drawn graphics, which is reliable for that, but it cannot tell us how *smooth* the 3D genuinely feels on a real phone's graphics chip. That's the one thing that still wants checking on an actual phone before calling this finished — if it feels sluggish on some phones, the honest fallback is to offer it only where the phone can comfortably run it, the same way a computer without a graphics card already gets the lighter "video-only" version automatically today.

**How we checked it.** 243 automatic checks, and the full browser-check suite (103 checks across a computer-sized and a phone-sized screen) pass, including new checks added specifically for this: the SCROLL button never grows bigger than a sensible share of a phone screen, the depth-gauge and brand line never collide on a short window, and the 3D tier now genuinely switches on for a phone when the phone's graphics support says it can.
...
**Verification (current, after the card deck, Sports, real Contact/About content, gold-by-default buttons and the journey reaching phones):** `tsc`, `eslint`, `prettier` clean; **243 unit tests**; **84 Playwright checks passed, 20 skipped by design, 0 failed** (each project skips tests that only apply to the other one) on the production build. Occasional flakes happen on this shared, software-rendering machine (1 in the latest full run, on a chat-timing check already given a generous 30 s allowance) and always pass on the automatic retry — see the Sports section's test-infrastructure note. Earlier, the whole scroll was watched frame-by-frame in real Edge, the chat was driven with good and bad answers, and the 3D scene's WebGL draw calls were counted (0 before the 3D phase, 15 in 1.5 s while visible in a software renderer, 0 after scrolling past). This machine has no graphics card of its own, so the journey's new `full` (3D) tier on a phone is checked here for layout only (`?journey=full` forces the tier for review) — not yet confirmed smooth on a real phone's graphics chip.
...
| **1F Hero → Surface** | ✅ | **Spike result:** in Edge/Chromium, dense-keyframe (`-g 2`) MP4 scrubbing seeks in a median 13–22 ms (p90 18–39 ms) and shows a new frame on ~88–90 % of animation ticks in both directions → *(originally `<video>` scrubbing was chosen. **Reversed 2026-09-21: the client still saw buffering on their machine, so the descent is now a canvas frame-sequence, see "Second smoothness pass" below.**)* Safari/Firefox/iOS **not yet tested (risk, Phase 4)**. `components/journey/Journey.tsx`: one tall pinned stage (8.5 screens), one progress value `P`, eased; timing lives in pure functions (`lib/journey/phases.ts`, unit-tested). Phase map: headline out 0–0.1 · loop→scrub 0.025–0.075 · scrub 0.075–0.5 · scrub→3D 0.46–0.54 · 3D path 0.5–1. Depth gauge, brand statement, ring-button (also covers the ✦ mark). **Revised 2026-09-22:** added a short line, **"Every Game Begins with the Ground"**, that fades in and out early in the descent — between the "THE ARENA" and "THE SURFACE" labels on the depth gauge, gone again before the brand statement takes its turn (`groundLineOpacity()` in `lib/journey/phases.ts`, same fade shape as the statement). It shares the statement's own on-screen spot (bottom-right) rather than the gauge's side, so it can never collide with the gauge's own labels. Tests: extended `tests/unit/journey.test.ts`. **Revised 2026-09-23 (client request: match the laptop on a phone too):** `Journey()`'s mode is now decided purely by `useCapability()` (`full`/`video`/`simple`) — the `useMediaQuery("(min-width: 900px)")` gate that used to force every phone to the plain fallback regardless of its actual graphics support is gone. A phone whose browser reports real hardware WebGL now gets the same `full` tier (3D turf) a capable laptop gets; one that only has software WebGL gets `video` (frame-sequence descent, no 3D) — the same tier a desktop without a graphics card already got; only an explicit `?journey=light` or a data-saver connection still gives the plain version. This is a genuine, informed reversal of the earlier width-based decision, not an oversight: brought the whole journey to a narrow screen for the first time, which surfaced several more instances of the *same* class of bug the hero headline had (content sized/positioned for a wide screen running into other content, or off the edge, on a narrow one) — all now fixed, in `components/journey/Journey.module.css` and `three/turf.module.css`:
  - **Round SCROLL/progress button:** its phone-fallback position (used once the ✦ mark crops off-screen on a narrow-tall video frame) used the same `maxRadius` as the desktop cap (90 px) — on a 390 px-wide phone that is a 180 px-wide button, big enough to sit over the last line of the hero headline. `cuePosition()` (`lib/media/watermark.ts`) now sizes that fallback from the *screen's* own smaller side, not from `maxRadius`. New unit test in `tests/unit/hero.test.ts`.
  - **Hero headline/tagline:** `clamp()` floors tuned for a laptop still overflowed a 390 px phone, and `.taglineBox`'s height grew (once it gained the Contact/Instagram buttons this session) past what `.headline`'s `svh`-based `bottom` had room for. Phone media query in `Journey.module.css`: smaller, fixed (not `svh`) font sizes and spacing, so neither depends on how tall the specific phone is.
  - **Depth gauge / brand statement / "Every Game Begins with the Ground":** the statement's `42vw` max-width was a sliver on a phone (163 px), wrapping into one word per line, with the last line reaching down into the gauge's own bottom label ("THE FIBRE"). Phone media query widens it to `82vw`, and shortens the gauge's own height so its bottom label sits clear of it.
  - **The 3D cross-section's layer labels** (`three/TurfSurface.tsx`/`turf.module.css`): drei's `<Html>` anchors each one in 3D world-space at the block's right edge; on a wide canvas that leaves room for the label to grow rightward, but on a narrow phone canvas that same point projects close to the screen's own edge, and the label (a leader line + name, `white-space: nowrap`) ran off screen — only the small gold dot was ever visible. Fixed by wrapping the label in an extra `.labelWrap` div and shifting *that* (not the label itself) `translateX(-100%)` on a phone, so it grows back toward the middle of the screen, dot still exactly on the anchor point. (It has to be a separate wrapper: the camera rig sets the label's own `transform` from JS every frame, which would otherwise silently overwrite a CSS `transform` placed on that same element — a real trap, worth remembering if this label ever needs touching again.)
  - **The "ILLUSTRATIVE · NOT TO SCALE" note:** shared the chat button's bottom-right corner on a phone; now sits bottom-left there instead.
  - **Deliberately left alone, same category of decision, different sections:** the Sports panel rail (`components/sections/Sports.tsx`) and the "How We Build" glass panel (`components/sections/HowWeBuild.tsx`) each still have their own, separate `useMediaQuery("(min-width: 900px)")` gate, untouched — this request was scoped to the opening scroll journey specifically.
  - **Not verifiable from this machine:** it has no graphics card, so every check above is a layout check (nothing cut off or overlapping, on software-drawn graphics) rather than a smoothness one. Real-device confirmation that the 3D genuinely feels smooth on a range of phones is still outstanding — see §8. |
...
| 17 | **Try the opening scroll journey (now including 3D) on a handful of real phones**, old and new, and say whether it feels smooth enough to leave switched on by default, or whether the `full` tier should be held back on weaker phones even when they report hardware WebGL support | Phase 2/4 | Stays on for any phone whose browser reports a real graphics card (current behaviour); nothing changes until told otherwise |
...
| Risk | Mitigation |
|---|---|
| Animation-heavy page runs poorly on real phones | Mount-on-intersect, one active canvas, capped DPR, mobile fallbacks, budgets enforced in CI. **Revised 2026-09-23:** the opening journey's `full`/3D tier now also reaches phones (client request), decided by the browser's own hardware-WebGL check rather than screen width — the fallback for a phone that genuinely can't do it (or explicitly says so, `?journey=light`/data-saver) is unchanged, but a phone that *reports* hardware support yet is still too weak in practice is an open risk (§8 #17); Sports' sideways rail and How We Build's glass panel deliberately were not extended the same way (still width-gated) |
